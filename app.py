...
def validate_inputs(inp: Inputs) -> list[str]:
    ...
    if inp.consortium_credit_today <= 0:
        errors.append("Carta de crédito deve ser positiva.")
    return errors
...
with st.sidebar.expander("📌 Gerais", expanded=True):
    ...
    investment_return_annual_net = st.number_input("Rentabilidade líquida a.a. (%)", value=10.2, step=0.1, format="%.2f")
...
try:
    ...
    with tab2:
        ...
        comp_df = pd.DataFrame({
            "Métrica": ["Patrimônio Total", "Patrimônio Financeiro", "Patrimônio Imobiliário", "Desembolso Total"],
            "À Vista": [
