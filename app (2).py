...
def simulate_cash_later(inp: Inputs) -> dict:
    ...
    ir_rate = inp.ir_aliquota_pct / 100.0
    ...
    for m in range(total_months + 1):
        ...
        if m > 0:
            ...
            ir_paid = investment_return * ir_rate
            balance += investment_return - ir_paid
            cumulative_ir_paid += ir_paid

            if (not purchased) and (m == purchase_month):
...
def simulate_consortium(inp: Inputs) -> dict:
    ...
    for m in range(total_months + 1):
        ...
        if m > 0:
            ...
            gross_return = balance * r_m
            ir_paid = gross_return * ir_rate
            balance += gross_return - ir_paid
            cumulative_ir_paid += ir_paid
        ...
        records.append({
            ...
            "real_estate_wealth": house_component,
            "total_wealth": balance + house_component,
            "remaining_debt": post_balance_debt if post_balance_debt is not None else 0.0,
        })
...
SENSITIVITY_PARAMS = {
    "investment_return_annual_net": "Rentabilidade bruta a.a. (%)",
    "ir_aliquota_pct": "Alíquota de IR (%)",
...
with st.sidebar.expander("📌 Gerais", expanded=True):
    ...
    investment_return_annual_net = st.number_input(
        ...
        help="Taxa bruta do investimento antes do IR. Ex: CDI bruto, Tesouro Selic, CDB bruto."
    )
    ir_aliquota_pct = st.number_input(
        ...
        help="IR incide sobre o rendimento bruto a cada mês. "
             "Referências: isento (LCI/LCA) = 0%; renda fixa >720 dias = 15%; "
             "renda fixa 361-720 dias = 17,5%; renda fixa 181-360 dias = 20%; até 180 dias = 22,5%."
    )
...
with st.sidebar.expander("💵 Estratégia: À Vista"):
    ...
    cash_purchase_extra_cost_pct = st.number_input("Custos extras na compra (%)", value=0.0, min_value=0.0, step=0.1, format="%.2f",
...
try:
    ...
    winner_label = "Consórcio" if diff > 0 else "Compra à Vista"
    winner_class = "winner-consortium" if diff > 0 else "winner-cash"
    winner_icon = "🏆"

    st.markdown(
        f'<div class="winner-box {winner_class}">'
        f'{winner_icon} Estratégia vencedora: <strong>{winner_label}</strong> '
    ...
    with tab1:
        ...
        fig.add_trace(go.Scatter(
            ...
            name="À Vista — Total", mode="lines",
            line=dict(color="#1f77b4", width=2.5),
            hovertemplate="Ano %{x:.1f}<br>Total: R$ %{y:,.0f}<extra>À Vista</extra>"
        ))
        fig.add_trace(go.Scatter(
            x=df_cash["year"], y=df_cash["financial_wealth"],
            name="À Vista — Caixa", mode="lines",
            line=dict(color="#1f77b4", width=1, dash="dot"),
            hovertemplate="Ano %{x:.1f}<br>Caixa: R$ %{y:,.0f}<extra>À Vista</extra>"
        ))
        fig.add_trace(go.Scatter(
        ...
        fig2.update_layout(
            title="Diferença de Patrimônio Total (Consórcio − À Vista)",
            xaxis_title="Ano",
            ...
            with col_s2:
                ...
                if crossover is not None:
                    st.success(
                        ...
                        f"— abaixo disso o **{'Consórcio' if sweep_results[0]['diff'] > 0 else 'À Vista'}** vence; "
                        f"acima, o **{'À Vista' if sweep_results[0]['diff'] > 0 else 'Consórcio'}** vence."
                    )
                else:
                    winner_sens = "Consórcio" if sweep_results[-1]["diff"] > 0 else "À Vista"
