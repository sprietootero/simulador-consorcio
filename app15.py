...
def missing_required_fields(inp: Inputs) -> list[str]:
    ...
    # Caixa inicial zero é um cenário válido quando o cliente tem aporte mensal,
    # principalmente em simulações de financiamento ou compra futura.
    if inp.initial_cash <= 0 and inp.monthly_savings <= 0:
        missing.append("Caixa inicial ou aporte mensal")
...
def validate_inputs(inp: Inputs) -> list[str]:
    ...
    if not (inp.cash_enabled or inp.cons_enabled or inp.financing_enabled):
        errors.append("Selecione pelo menos uma estratégia para comparar.")

    if inp.horizon_years <= 0:
        errors.append("Horizonte deve ser maior que zero.")

    # Validações da estratégia à vista apenas quando ela está habilitada.
    if inp.cash_enabled and not inp.no_cash_purchase:
    ...
    # Validações do consórcio apenas quando ele está habilitado.
    if inp.cons_enabled:
...
def build_executive_summary(inp: Inputs, label_cash: str,
    ...
    if not candidates:
        return {
            ...
            "advantage_abs": 0.0,
            "advantage_pct": 0.0,
            "alerts": ["Nenhuma estratégia foi habilitada para comparação."],
        }
...
def build_pdf_report(inp: Inputs,
                     label_cash: str,
                     summary: dict,
                     ranking_rows: list[tuple[str, float, float]],
                     cash_result: Optional[dict],
                     cons_result: Optional[dict],
                     fin_result: Optional[dict]) -> bytes:
    """
    Gera um PDF de uma página com o relatório final da simulação.

    Layout:
    - Cabeçalho com título e data
    - Bloco "Estratégia recomendada" com vencedor e vantagem
    - Tabela de premissas (horizonte, imóvel, rentabilidade, etc.)
    ...
    doc = SimpleDocTemplate(
        ...
        title="Relatório Final - Simulação Imobiliária",
    )
    ...
    # Mapa de cores por estratégia (consistente com a UI)
    color_map = {
        "winner-cash":       colors.HexColor("#0077a8"),
        "winner-consortium": colors.HexColor("#f0a500"),
        "winner-financing":  colors.HexColor("#28a745"),
    }
    winner_color = color_map.get(summary.get("winner_class", ""), colors.HexColor("#1a1a2e"))
    winner_color_hex = "#" + winner_color.hexval()[2:]  # 0xRRGGBB → #RRGGBB

    story = []

    # === Cabeçalho ===
    story.append(Paragraph("Relatório Final — Simulação Imobiliária", title_style))
    ...
    # === Bloco do vencedor ===
    if summary.get("winner_label"):
        winner_text = (
            f'<font size="11" color="#64748b"><b>ESTRATÉGIA RECOMENDADA</b></font><br/>'
            f'<font size="20" color="{winner_color_hex}"><b>{summary["winner_label"]}</b></font><br/>'
            f'<font size="10" color="#475569">'
            f'Patrimônio total projetado: <b>{fmt_brl(summary["winner_value"])}</b>'
            f'</font>'
        )
        if summary.get("runner_up_label"):
            adv_pct_str = f"{summary['advantage_pct']:+.1f}%"
            winner_text += (
                f'<br/><font size="10" color="#475569">'
                f'Vantagem sobre {summary["runner_up_label"]}: '
                f'<b>{fmt_brl(summary["advantage_abs"])}</b> ({adv_pct_str})'
                f'</font>'
            )

        winner_box = Table(
            [[Paragraph(winner_text, body_style)]],
            colWidths=[doc.width],
        )
        winner_box.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#f8fafc")),
            ("BOX", (0, 0), (-1, -1), 1.2, winner_color),
    ...
    story.append(Paragraph("Premissas do cenário", h2_style))
    premissas_data = [
        ["Parâmetro", "Valor"],
        ["Horizonte da análise", f"{inp.horizon_years} anos"],
        ["Caixa inicial", fmt_brl(inp.initial_cash)],
        ["Aporte mensal", fmt_brl(inp.monthly_savings)],
        ["Preço do imóvel hoje", fmt_brl(inp.property_price_today)],
        ["Valorização anual do imóvel", f"{inp.property_appreciation_annual:.2f}% a.a."],
        ["Rentabilidade líquida", f"{inp.investment_return_annual_net:.2f}% a.a."],
    ]
    if inp.cons_enabled:
        premissas_data.append(["Carta de crédito (consórcio)", fmt_brl(inp.consortium_credit_today)])
        premissas_data.append(["Lance / Ano contemplação", f"{inp.bid_pct:.0f}% / Ano {inp.contemplation_year:.1f}"])
    if inp.financing_enabled:
        premissas_data.append([
            "Financiamento",
            f"{inp.financing_system} • Entrada {inp.financing_down_payment_pct:.0f}% • "
            f"Juros {inp.financing_interest_annual:.2f}% a.a. • {inp.financing_term_months} meses",
        ])

    premissas_table = Table(premissas_data, colWidths=[doc.width * 0.45, doc.width * 0.55])
    premissas_table.setStyle(TableStyle([
    ...
    story.append(premissas_table)

    # === Ranking comparativo ===
    story.append(Paragraph("Ranking comparativo", h2_style))
    ranking_data = [["Posição", "Estratégia", "Patrimônio Final", "Desembolso do Bolso"]]
    for idx, (name, total, out) in enumerate(ranking_rows):
        marker = "1º (vencedora)" if idx == 0 else f"{idx + 1}º"
        ranking_data.append([marker, name, fmt_brl(total), fmt_brl(out)])

    ranking_table = Table(
        ranking_data,
        colWidths=[doc.width * 0.13, doc.width * 0.32, doc.width * 0.275, doc.width * 0.275],
    )
    ranking_style = [
        ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#1e293b")),
        ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
        ...
        ("BOTTOMPADDING", (0, 0), (-1, -1), 6),
        ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
        ("ALIGN", (2, 1), (-1, -1), "RIGHT"),  # alinha colunas R$ à direita
        ("ALIGN", (0, 1), (0, -1), "CENTER"),  # posição centralizada
    ]
    # Destaca a linha do vencedor com fundo amarelo claro
    if len(ranking_data) > 1:
        ranking_style.append(
    ...
    if summary.get("alerts"):
        story.append(Paragraph("Pontos de atenção", h2_style))
    ...
    story.append(Paragraph(
        "Documento gerado por Simulador Imobiliário. "
        "Os valores são projeções baseadas nas premissas informadas e não constituem garantia de resultado.",
        footer_style,
    ))
...
<style>
    ...
    .ui-chip-wrap {display:flex; gap:.5rem; flex-wrap:wrap; margin:.35rem 0 0.9rem 0;}
    .ui-chip {background:#f1f5f9; border:1px solid #dbe3ee; color:#334155; padding:.38rem .65rem; border-radius:999px; font-size:.82rem; font-weight:600;}
    .helper-box {background:linear-gradient(135deg,#f8fafc 0%,#eef2ff 100%); border:1px solid #dbeafe; border-radius:12px; padding:.9rem 1rem; margin:.3rem 0 1rem 0; color:#334155; font-size:.9rem; box-shadow:0 2px 8px rgba(15,23,42,.05);}
    .section-title {background:linear-gradient(135deg,#0f172a 0%,#1e293b 100%); color:#f8fafc; border-radius:10px; padding:.45rem .7rem; font-size:.8rem; text-transform:uppercase; letter-spacing:.06em; margin:.8rem 0 .5rem 0;}
...
st.markdown(
    "<div class='helper-box'><strong>Como usar:</strong> 1) preencha os dados na barra lateral, "
    "2) habilite as estratégias, 3) compare patrimônio final no cartão executivo e na aba de gráficos.</div>",
    unsafe_allow_html=True,
)
...
# =============================================================================
st.sidebar.markdown("<div class='section-title'>1️⃣ Cliente</div>", unsafe_allow_html=True)
...
# =============================================================================
st.sidebar.markdown("<div class='section-title'>2️⃣ Imóvel</div>", unsafe_allow_html=True)
...
# =============================================================================
st.sidebar.markdown("<div class='section-title'>3️⃣ Estratégias</div>", unsafe_allow_html=True)
...
# Resumo rápido da simulação na sidebar
strategies_on = [name for name, enabled in [
    ("À Vista", cash_enabled),
    ("Consórcio", cons_enabled),
    ("Financiamento", financing_enabled),
] if enabled]

st.sidebar.markdown("<div class='section-title'>✅ Resumo rápido</div>", unsafe_allow_html=True)
st.sidebar.markdown(
    f"""
<div class='ui-chip-wrap'>
  <span class='ui-chip'>Horizonte: {int(horizon_years)} anos</span>
  <span class='ui-chip'>Imóvel: {fmt_brl(property_price_today)}</span>
  <span class='ui-chip'>Ativas: {', '.join(strategies_on) if strategies_on else 'Nenhuma'}</span>
</div>
""",
    unsafe_allow_html=True,
)
...
# =============================================================================
if modo_avancado:
    st.sidebar.markdown("<div class='section-title'>4️⃣ Configurações</div>", unsafe_allow_html=True)
...
try:
    ...
    tab1, tab2, tab3, tab4, tab5, tab_report = st.tabs([
        "📊 Gráfico", "📝 Resumo", "🔎 Auditoria", "📅 Ano a Ano", "📉 Sensibilidade", "📄 Relatório Final"
    ])
    ...
    with tab3:
        ...
        if fin_result is not None:
            ...
            # Aviso quando amortizações foram puladas por falta de caixa
            skipped = fin_result.get("prepayments_skipped", 0)
            if skipped > 0:
                st.warning(
                    f"⚠️ **{skipped}** amortização(ões) antecipada(s) foram **puladas** "
                    f"durante a simulação porque o caixa não comportava o valor no mês agendado. "
                    f"Para que ocorram, considere aumentar o caixa inicial, o aporte mensal, "
                    f"ou diminuir o valor / espaçar a periodicidade."
                )
    ...
    with tab_report:
        ...
        st.caption("Resumo executivo pronto para apresentação ao cliente. Use o botão abaixo para exportar em PDF.")

        ranking_rows = []
        if cash_result is not None:
            ranking_rows.append((label_cash, cash_result["final_total_wealth"], cash_result["out_of_pocket_total"]))
        if cons_result is not None:
            ranking_rows.append(("Consórcio", cons_result["final_total_wealth"], cons_result["out_of_pocket_total"]))
        if fin_result is not None:
            ranking_rows.append(("Financiamento", fin_result["final_total_wealth"], fin_result["out_of_pocket_total"]))

        ranking_rows.sort(key=lambda x: x[1], reverse=True)

        col_r1, col_r2, col_r3 = st.columns(3)
        with col_r1:
            st.metric("Estratégia recomendada", ranking_rows[0][0])
        with col_r2:
            st.metric("Patrimônio final projetado", fmt_brl(ranking_rows[0][1]))
        with col_r3:
            if len(ranking_rows) > 1:
                st.metric("Vantagem sobre 2º lugar", fmt_brl(ranking_rows[0][1] - ranking_rows[1][1]))
            else:
                st.metric("Vantagem sobre 2º lugar", "N/A")
        ...
        ranking_df = pd.DataFrame([
            {
                "Posição": idx + 1,
                "Estratégia": name,
                "Patrimônio Final": fmt_brl(total),
                "Desembolso do Bolso": fmt_brl(out),
            }
            for idx, (name, total, out) in enumerate(ranking_rows)
        ])
        ...
        try:
            pdf_bytes = build_pdf_report(
                inp, label_cash, summary, ranking_rows,
                cash_result, cons_result, fin_result,
            )
            timestamp = datetime.now().strftime("%Y%m%d_%H%M")
            st.download_button(
                "⬇️ Baixar relatório (PDF)",
                data=pdf_bytes,
                file_name=f"relatorio_simulacao_{timestamp}.pdf",
